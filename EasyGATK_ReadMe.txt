...
EasyGATK is a python based wrapper script designed to help researchers set-up and perform WES analyses using the GATK pipeline. 

There are several files in this package:

	1.	configuration.txt : to set-up software path, input files, references, etc.
	2.	EasyGATK_v1.0.py : the main script of EasyGATK
	...
	4.	haplotypeCaller_minpruneEstimate.py : to automatically assign minPruning values based on sequencing depth
		...
		(in template folder)
	5.	template1_mapping.sh : an example of commands generated by EasyGATK for step 1 (see details in the "configuration.txt" file)
	6.	template2_Bam_PreProcessing.sh : an example of commands generated by EasyGATK for step 2
	7.	template3 _checkCoverage.sh : an example of commands generated by EasyGATK for step 3
...
1.	Set paths and parameters in "configuration.txt":
	-	Most default values and examples have already been provided
	-	Use the absolute path for software tools
		Example : /usr/username/data/GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar
	-	Use the absolute path for working directory and input sample directory
...
4.	Run the .sh file :
	-	Users may directly run these shell script files one by one on terminal using a computing node:
		-	make the ".sh" file executable: chmod a+x /path/to/Step1_MappingReads_sample_name.sh
		-	and then type: ./Step1_MappingReads_sample_name.sh
...
1.	To decide minPruning value:
	-	If you do not know the depth of coverage of your samples, please use option “yes” on minprune_estimate in "configuration.txt". With "minprune_estimate = yes", EasyGATK will ignore the default minPruning value in "configuration.txt" and create "Step3_CheckCoverage_sample_name.sh". The Step4_ VariantCalling_sample_name.sh will be created after Step3 is done.

	-	If you know the depth of coverage or want to manually set the minPruning value, you can use "minprune_estimate = no" in "configuration.txt" and manually assign a minPruning value. In this case the "Step3" script will not be created. Please proceed from "Step2" directly to "Step4".
